...
export default class ProfileScreen extends Component {
  ...
  ownerSwitchButton = () => {
    this.setState(prevState => ({
      ownerSwitch: !prevState.ownerSwitch,
    }));
    Actions.tabbar2();
  }

  muteNotificationButton = () => {Action
    this.setState(prevState => ({
  ...
  render(){
    ...
    const showAlert = () => {
      Alert.alert(
        'Are you sure to update these changes?',
        '',
        [
      ...
      return(
        <ScrollView style={styles.UserProfileContainer}>
          ...
          <View style={{ flexDirection: 'row', paddingTop: 10 }}>
            <Text style={{ flex: 3,  marginLeft: 20, fontSize: 18 }}>Switch to Owner Profile</Text>
            {
              this.state.ownerSwitch ? 
          ...
          <View style={{ margin : 20, marginBottom: 0, marginBottom: 10 }}>
            <View style={{ flexDirection: 'row', marginBottom:  15 }}>
                <Text style={{ flex: 3, fontWeight: 'bold' }}>Password</Text>
                  <Icon.Button name="pencil" color="white" backgroundColor="green" style={{ marginLeft: 5, paddingBottom: 5, flex: 1 }} onPress={() => Actions.push('signup', { oldPassword : userPassword, userEmail : userEmail })}>
                    Change Password
                  </Icon.Button>
              </View>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20, }}>
              <View style={{ flex: 1, flexDirection: 'row' }}>
                ...
                <Text style={{ fontWeight: 'bold', marginLeft: 10, fontSize:18 }}>Account Settings</Text>
              </View>
