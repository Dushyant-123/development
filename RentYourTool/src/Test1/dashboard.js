...
export default class MyProject extends Component {
  ...
  getToolList() {
    console.log('Get Tool List ')
    NetInfo.isConnected.fetch().then(isConnected => {
      if(isConnected) {
        this.setState({
          IsLoaderVisible: true
        })
        var URL=config.BASE_URL+'ToolsConfiguration/GetToolList?token=TDhCTDhYbG5XMmRNbW96RmlPT2ZBTXhyWFhwUzNtWUhqNkFtMC9renhLWT06dGlrYW1AZ21haWwuY29tOjYzNjY1MjU5NTM3NTA5NzA2MTow&RentelID='+this.state.userId;
        console.log(URL, 'Dashboard Url')
          fetch(URL, {
            ...
            console.log(responseJson, 'Dashboard Response')
  ...
  render() {
    ...
    const Header = () => (
      <View style={styles.header}>
        {
          this.state.searchTextInput ? <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: headerBackgroundColor }}>
          <Icon.Button size={15} name="chevron-left" backgroundColor="transparent" onPress={ () => this.changeToSearch() }/>
          <TextInput 
          ...
          />
        <TouchableOpacity onPress={() => this.SearchFilterFunction()}><Text>Search</Text></TouchableOpacity>
        </View> : <View style={{ height: 55, flexDirection: 'row', alignItems:'center', backgroundColor: headerBackgroundColor }}>
            <View style={{ flex: 1 }} />
            <View style={{ flex: 1, alignItems: 'center'}}>
              <Image source={ require('../images/icon.png') } style={{ width: 32, height: 32 }} />
            </View>
            <View style={{ flex: 1, alignItems: 'flex-end' }} >
              <Icon.Button name="search" color="white" size={25} backgroundColor="transparent" onPress={ () => this.changeToSearch() } />
            </View>
          </View>
        }
          ...
          renderContent={
            <View style={{ paddingBottom : 10 }}>
              <ListView
                  ...
                  renderRow={(rowData) => <ListViewComponent navigation={this.props.navigation} token={this.state.token} rowData={rowData} startTime={this.state.startTime} endTime={this.state.endTime} duration={duration} zipCode="201014"/>}
